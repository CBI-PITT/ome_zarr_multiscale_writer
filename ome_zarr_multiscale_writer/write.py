...
def write_ome_zarr_multiscale(
    ...
    shard_shape = (
        shard_shape or data.shape
    )  # default to full shape if not provided, only applies to Zarr v3
    ...
    if compressor:
        compressor_obj = BloscCodec(
            cname=compressor, clevel=compression_level, shuffle=BloscShuffle.bitshuffle
        )
    else:
        compressor_obj = None
...
def generate_multiscales_from_omezarr(
    ...
    if hasattr(source_group, "keys"):
        level_keys = [k for k in source_group.keys() if str(k).isdigit()]
    else:
        # For zarr.Array or other types, fall back to checking if '0' exists
        level_keys = ["0"] if "0" in str(source_group) else []
...
if __name__ == "__main__":
    ...
    output = "/tmp/output_example.ome.zarr"  # Example path - customize as needed
    ...
    class FakeData:
        ...
        def __init__(self, shape: Tuple[int, int, int]):
            ...
            self.data = np.random.randint(
                0, 65535, size=(self.shape[1], self.shape[2]), dtype=np.uint16
            )
    ...
    # data = FakeData((2000, 20000, 24000))  # Use FakeData for incremental writing
    data = np.random.randint(
        0, 65535, size=(200, 2000, 2400), dtype=np.uint16
    )  # Small example array
